    ...
    * */
    ...
    magnify.prototype = {
        init: function (ele) {
            ...
            // 如果是默认照片就不出现弹窗
            if(imgSrc==='./img/moren.png'){
                return
            }
            this.groupName = null;
            var currentGroupName = $(ele).data('group');
            // 图片id
            // var companyid = $(ele).data('companyid');
            // console.log(companyid,'companyidcompanyidcompanyid')


            var groupList = $D.find('[data-group="' + currentGroupName + '"]');
            //groupList.length 照片的数量
            // console.log(groupList.length,'groupList.lengthgroupList.lengthgroupList.length')
            ...
            // 传入照片长度来控制是否能切换
            this.open(currentGroupName,groupList.length);
            this.loadImg(imgSrc, currentGroupName);
        ...
        // 属性
        open: function (currentGroupName,length) {
            ...
            this.addEvents(currentGroupName);
            // 模态窗的定位
            this.setModalPos(this.$magnify, currentGroupName);
            this.triggerHook('opened', this.$ele);
        },
        build: function (length) {
            var _html = this.createDOM(length);
            var $magnify = $(_html);
            //获取所有magnify DOM元素
        ...
        createDOM: function (length) {
            ...
            if(length==1){
                ...
                '<div class="magnify-footer"><div class="magnify-toolbar"></div></div>' +
                '</div>';
            }else{
                return '<div class="magnify-modal">' +
                '<div class="magnify-header">' + this.createTitle() +
        ...
        setModalPos: function (modal, currentGroupName) {
            // console.log(currentGroupName, '模态窗定位')
            var winWidth = $W.width(),
                winHeight = $W.height(),
                modalSize = this.settings.modalSize,
                modalWidth = modalSize[0],
                modalHeight = modalSize[1];
            if(currentGroupName=='g9'||currentGroupName=='g10'||currentGroupName=='g16'||currentGroupName>=200){
                modal.css({
                    "z-index":100000,
        ...
        setImageSize: function (img, tag) {
            // console.log(tag,'33333',this.isRotated,img)
        ...
        addEvents: function (currentGroupName) {
            var self = this;
        ...
        wheel: function (e, self) {
            // if (self.groupName == 'g5') return
            console.log(e, '滚轮', self)
            stopPre(e);
            var delta = 1;
        ...
        jump: function (n,currentGroupName) {
            ...
            console.log(currentGroupName,'上下页')
            if(currentGroupName=='g5'){
                this.loadImg(this.groupData[n].src, 'g5');
            }else{
